...
class NativeCanvasView: UIView {
    ...
    /// 缩放范围
    private let minZoomScale: CGFloat = 0.3
    ...
    /// 全局 zIndex 计数器 - 所有对象类型共享
    private var globalZIndexCounter: Int = 0
    ...
    private func handleImageSelected(url: String, at location: CGPoint) {
        ...
        loadImageForSize(from: url) { [weak self] originalSize in
            ...
            // 创建图片图层节点（使用全局 zIndex 确保正确的层级顺序）
            let imageLayer = LayerNode(
                ...
                rotation: 0,
                isLocked: false,
                zIndex: self.getNextGlobalZIndex(),
                opacity: 1.0,
                createdAt: Date()
            )
    ...
    /// 获取下一个全局 Z-Index（所有对象类型共享）
    func getNextGlobalZIndex() -> Int {
        globalZIndexCounter += 1
        return globalZIndexCounter
    }
    
    /// 更新全局 Z-Index 计数器（在加载数据时调用）
    func updateGlobalZIndexCounter() {
        // 获取所有对象的最大 zIndex
    ...
    /// 这是计算图片添加位置的核心方法
    /// 使用 UIScrollView 的标准公式：contentCoord = (viewportCoord + offset) / scale
